...
def create_folders():
    ...
    for i in newpaths:
        try:
            ...
            info_label.configure(text="An error occurred, please check if the directory is writable.")
        else:
...
directory_frame.pack(pady=20, padx=20, fill="x")

root_path_entry = ctk.CTkEntry(directory_frame, width=400)
root_path_entry.insert(0, default_path)
root_path_entry.pack(side="left", padx=10)

choose_dir_button = ctk.CTkButton(directory_frame, text="Choose Directory", command=choose_directory)
choose_dir_button.pack(side="left")
...
template_combobox.pack(pady=10, padx=20, fill="x")

# Create Folders Button
create_folders_button = ctk.CTkButton(app, text="Create Folders", command=create_folders)
create_folders_button.pack(pady=20, padx=20, fill="x")

# Copyright and Version Labels
copyright_label = ctk.CTkLabel(app, text="© 2025 Overmind Studios - Kummer & Gerhardt GbR")
copyright_label.pack(pady=(0, 5))

version_label = ctk.CTkLabel(app, text="version " + version)
version_label.pack()
...
info_window.withdraw()  # Hide the window initially
